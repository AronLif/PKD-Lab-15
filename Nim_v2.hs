...
genGameState = do
  putStrLn "Enter (x,y) where x and y are the lowest and highest amount of stones you would like to put in each pile"
  (x,y) <- readMove 
  random1 <- getStdRandom (randomR (x,y))
  random2 <- getStdRandom (randomR (x,y))
  random3 <- getStdRandom (randomR (x,y))
...
main = do 
  ...
  let loop = do putStr "Type QUIT to quit or PLAY to play: "
                str <- getLine
                if str == "QUIT"
                  then return ()
                  else if str == "PLAY"
